...
def events_to_image_torch(xs, ys, ps,
        device=None, sensor_size=(128, 128), clip_out_of_range=True,
...
def events_to_image(xs, ys, ps, sensor_size=(128, 128), interpolation=None, padding=False, meanval=False, default=0):
    """
...
def plot_events(data, save_path, sensor_size):
    xs, ys, ts, ps = data[:, 0], data[:, 1], data[:, 2], data[:, 3] #*2-1

    img = events_to_image(xs.astype(int), ys.astype(int), ps, sensor_size, interpolation=None, padding=False, meanval=False)
    
    mn, mx = np.min(img), np.max(img)
    img = (img-mn)/(mx-mn)*255

    # img = np.rot90(img, k=1) #rotar
    # img = np.flip(img, axis=1) #reflejar

    # Convertir el arreglo NumPy en una imagen PIL
    img_pil = Image.fromarray(np.uint8(img), 'L')
...
if __name__ == "__main__":
    ...
    #'''
    path_aedat = '../N-Caltech101/N-Caltech101_npy/'
    path_frame = '../N-Caltech101/event_frames_acc/'
